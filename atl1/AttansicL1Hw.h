#ifndef _ATTANSICL1HW_H_
#define _ATTANSICL1HW_H_
...
{
	/* rx */
	UInt32 rx_ok;		/* The number of good packet received. */
	UInt32 rx_bcast;		/* The number of good broadcast packet received. */
	UInt32 rx_mcast;		/* The number of good multicast packet received. */
	UInt32 rx_pause;		/* The number of Pause packet received. */
	UInt32 rx_ctrl;		/* The number of Control packet received other than Pause frame. */
	UInt32 rx_fcs_err;		/* The number of packets with bad FCS. */
	UInt32 rx_len_err;		/* The number of packets with mismatch of length field and actual size. */
	UInt32 rx_byte_cnt;	/* The number of bytes of good packet received. FCS is NOT included. */
	UInt32 rx_runt;		/* The number of packets received that are less than 64 byte long and with good FCS. */
	UInt32 rx_frag;		/* The number of packets received that are less than 64 byte long and with bad FCS. */
	UInt32 rx_sz_64;		/* The number of good and bad packets received that are 64 byte long. */
	UInt32 rx_sz_65_127;	/* The number of good and bad packets received that are between 65 and 127-byte long. */
	UInt32 rx_sz_128_255;	/* The number of good and bad packets received that are between 128 and 255-byte long. */
	UInt32 rx_sz_256_511;	/* The number of good and bad packets received that are between 256 and 511-byte long. */
	UInt32 rx_sz_512_1023;	/* The number of good and bad packets received that are between 512 and 1023-byte long. */
	UInt32 rx_sz_1024_1518;	/* The number of good and bad packets received that are between 1024 and 1518-byte long. */
	UInt32 rx_sz_1519_max;	/* The number of good and bad packets received that are between 1519-byte and MTU. */
	UInt32 rx_sz_ov;		/* The number of good and bad packets received that are more than MTU size šC truncated by Selene. */
	UInt32 rx_rxf_ov;		/* The number of frame dropped due to occurrence of RX FIFO overflow. */
	UInt32 rx_rrd_ov;		/* The number of frame dropped due to occurrence of RRD overflow. */
	UInt32 rx_align_err;	/* Alignment Error */
	UInt32 rx_bcast_byte_cnt;	/* The byte count of broadcast packet received, excluding FCS. */
	UInt32 rx_mcast_byte_cnt;	/* The byte count of multicast packet received, excluding FCS. */
	UInt32 rx_err_addr;	/* The number of packets dropped due to address filtering. */

	/* tx */
	UInt32 tx_ok;		/* The number of good packet transmitted. */
	UInt32 tx_bcast;		/* The number of good broadcast packet transmitted. */
	UInt32 tx_mcast;		/* The number of good multicast packet transmitted. */
	UInt32 tx_pause;		/* The number of Pause packet transmitted. */
	UInt32 tx_exc_defer;	/* The number of packets transmitted with excessive deferral. */
	UInt32 tx_ctrl;		/* The number of packets transmitted is a control frame, excluding Pause frame. */
	UInt32 tx_defer;		/* The number of packets transmitted that is deferred. */
	UInt32 tx_byte_cnt;	/* The number of bytes of data transmitted. FCS is NOT included. */
	UInt32 tx_sz_64;		/* The number of good and bad packets transmitted that are 64 byte long. */
	UInt32 tx_sz_65_127;	/* The number of good and bad packets transmitted that are between 65 and 127-byte long. */
	UInt32 tx_sz_128_255;	/* The number of good and bad packets transmitted that are between 128 and 255-byte long. */
	UInt32 tx_sz_256_511;	/* The number of good and bad packets transmitted that are between 256 and 511-byte long. */
	UInt32 tx_sz_512_1023;	/* The number of good and bad packets transmitted that are between 512 and 1023-byte long. */
	UInt32 tx_sz_1024_1518;	/* The number of good and bad packets transmitted that are between 1024 and 1518-byte long. */
	UInt32 tx_sz_1519_max;	/* The number of good and bad packets transmitted that are between 1519-byte and MTU. */
	UInt32 tx_1_col;		/* The number of packets subsequently transmitted successfully with a single prior collision. */
	UInt32 tx_2_col;		/* The number of packets subsequently transmitted successfully with multiple prior collisions. */
	UInt32 tx_late_col;	/* The number of packets transmitted with late collisions. */
	UInt32 tx_abort_col;	/* The number of transmit packets aborted due to excessive collisions. */
	UInt32 tx_underrun;	/* The number of transmit packets aborted due to transmit FIFO underrun, or TRD FIFO underrun */
	UInt32 tx_rd_eop;		/* The number of times that read beyond the EOP into the next frame area when TRD was not written timely */
	UInt32 tx_len_err;		/* The number of transmit packets with length field does NOT match the actual frame size. */
	UInt32 tx_trunc;		/* The number of transmit packets truncated due to size exceeding MTU. */
	UInt32 tx_bcast_byte;	/* The byte count of broadcast packet transmitted, excluding FCS. */
	UInt32 tx_mcast_byte;	/* The byte count of multicast packet transmitted, excluding FCS. */
	UInt32 smb_updated;	/* 1: SMB Updated. This is used by software as the indication of the statistics update.
...
{
	UInt32 int_stats;		/* interrupt status */
	UInt16 rrd_prod_idx;	/* TRD Producer Index. */
	UInt16 rfd_cons_idx;	/* RFD Consumer Index. */
	UInt16 update;		/* Selene sets this bit every time it DMA the CMB to host memory.
				 * Software supposes to clear this bit when CMB information is processed. */
	UInt16 tpd_cons_idx;	/* TPD Consumer Index. */
} __attribute__ ((packed));

/* RRD descriptor */
struct rx_return_desc	 
{
	UInt8 num_buf;		/* Number of RFD buffers used by the received packet */
	UInt8 resved;
	UInt16 buf_indx;		/* RFD Index of the first buffer */
	union 
	{
		UInt32 valid;
		struct 
	...
	UInt16 pkt_flg;		/* Packet flags */
	UInt16 err_flg;		/* Error flags */
	UInt16 resved2;
	UInt16 vlan_tag;		/* VLAN TAG */
} __attribute__ ((packed));
...
struct rx_free_desc  
{
	UInt64 buffer_addr;		/* Little Endian. Address of the descriptor's data buffer */
	UInt16 buf_len;			/* Little Endian. Size of the receive buffer in host memory, in byte */
	UInt16 coalese;			/* Little Endian. Update consumer index to host after the reception of this frame */
} __attribute__ ((packed));
...
struct tso_param 
{
	...
	UInt32 tsopu;      /* tso_param upper word */
    UInt32 tsopl;      /* tso_param lower word */
};

struct csum_param  
{
	/* The order of these declarations is important -- don't change it */
	UInt32 csumpu;     /* csum_param upper word */
	UInt32 csumpl;     /* csum_param lower word */
};
...
{
	UInt64 buffer_addr;		//Little Endian
	tpd_descr desc;
};
...
struct atl1_spi_flash_dev  
{
	...
	UInt8 cmd_rdsr;
	UInt8 cmd_rdid;
	UInt8 cmd_sector_erase;
	UInt8 cmd_chip_erase;
};
