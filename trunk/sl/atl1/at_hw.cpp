...
/* check_cable:
 * return 
 *		FALSE:disconnect, TRUE:connected
 */
boolean_t 
check_cable(struct at_hw *hw)
{
	u16 PairNo, Index;
	u16 PhyValue;
	...
	at_write_phy_reg(hw, MII_BMCR, PhyValue);
	
	for (PairNo=0; PairNo<4; PairNo++)
	{
		PhyValue = (u16)(PairNo << CDTC_PAIR_OFFSET );
		PhyValue |= CDTC_EN ;
		
		at_write_phy_reg(hw, MII_CDTC, PhyValue);
		
		// wait 200ms atmost
		for (Index=0; Index<200; Index++)
		{
			msec_delay(1);
			at_read_phy_reg(hw, MII_CDTC, &PhyValue);
			if (0 == (PhyValue&CDTC_EN))
				break;
		}
		
		msec_delay(1);
		at_read_phy_reg(hw, MII_CDTS, &PhyValue);
		
		if (CDTS_OPEN != ((PhyValue >> CDTS_STATUS_OFFSET)&3))
			return TRUE;
	}
	return FALSE;
...
at_read_mac_addr(struct at_hw * hw)
{
    ...
    if (get_permanent_address(hw)) {
        ...
        hw->perm_mac_addr[4] = 0x11;
        hw->perm_mac_addr[5] = 0x08;
    } 
...
{    
    ...
    // RESET GPHY Core
    AT_WRITE_REGW(hw, REG_GPHY_ENABLE, 0);
    msec_delay(2);
    AT_WRITE_REGW(hw, REG_GPHY_ENABLE, 1);
    msec_delay(2);
    
    // check the cable exist ?
    if(!check_cable(hw)) { // disconnect
		// create a fake plug action
	...
	if (test_bit(0, &hw->force_ps)) {
		at_write_phy_reg(hw, MII_DBG_ADDR, 0);
		at_write_phy_reg(hw, MII_DBG_DATA, 0x124E);
		at_write_phy_reg(hw, MII_DBG_ADDR, 1);
    	at_read_phy_reg(hw, MII_DBG_DATA, &phy_data);
    	phy_data |= 0x3;
    	at_write_phy_reg(hw, MII_DBG_DATA, phy_data);
		
    	for (i=0; i < 15; i++) {
        	msec_delay(100); // 1.5s.
    	}
    	at_write_phy_reg(hw, MII_DBG_ADDR, 0);
    	at_write_phy_reg(hw, MII_DBG_DATA, 0x024E);
    }
...
void at_force_ps(struct at_hw* hw)
{
	at_write_phy_reg(hw, MII_DBG_ADDR, 0);
	at_write_phy_reg(hw, MII_DBG_DATA, 0x124E);
	at_write_phy_reg(hw, MII_DBG_ADDR, 2);
	at_write_phy_reg(hw, MII_DBG_DATA, 0x3000);
	at_write_phy_reg(hw, MII_DBG_ADDR, 3);
	at_write_phy_reg(hw, MII_DBG_DATA, 0);
}
