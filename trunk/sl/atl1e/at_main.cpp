...
at_setup_ring_resources(at_adapter *adapter)
{
	...
	DbgPrint("dma registers ring=%X tpd_ring=%X rxf_page[0][0]=%X rxf_page[0][1]=%X tpd_cmb=%X tpd=%p \n",
			(u32)adapter->ring_dma,  (u32)adapter->tpd_ring_dma,
			(u32)adapter->rxf_page[0][0].dma, (u32)adapter->rxf_page[0][1].dma,
			(u32)adapter->tpd_cmb_dma, adapter->tpd_cmb);      
