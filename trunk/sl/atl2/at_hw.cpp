...
void at_init_pcie(at_hw *hw)
{
    ...
    AT_WRITE_REG(hw, REG_PCIE_DLL_TX_CTRL1, value);
...
at_phy_init(at_hw* hw)
{
	...
	at_read_phy_reg(hw, MII_DBG_DATA, &phy_val);
	
	/* 024E / 124E 0r 0274 / 1274 ? */
	if (phy_val & 0x1000) {
		phy_val &= ~0x1000;
		at_write_phy_reg(hw, MII_DBG_DATA, phy_val);
	}
...
bool 
spi_read(at_hw* hw, u32 addr, u32* buf)
{
    int i;
    u32 value;
...
void at_force_ps(at_hw* hw)
{
	...
	at_write_phy_reg(hw, MII_DBG_ADDR, 0);
	at_read_phy_reg(hw, MII_DBG_DATA, &phy_val);
	at_write_phy_reg(hw, MII_DBG_DATA, phy_val|0x1000);
